...
class Fun(object):
    ...
    def privatemenssagem (self, message, name_sender):
        try:
            ...
            if not '/leave' in sendM:
                self.post(message=sendM, to=usuario)
        except Exception:
            pass

    def cotacao(self):
        response = requests.get("https://economia.awesomeapi.com.br/all/USD").json()
        dolar = response["USD"]["bid"][:4]
        self.post(message=f"1 Dólar e igual R${dolar}")